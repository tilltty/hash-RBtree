...
queue<pair<string, pair<uint64_t, int> > > BlockIndexRBTree::checkBackups(BlockIndexRBTree* root) {
	return checkBackups1(root->lson);
}
queue<pair<string, pair<uint64_t, int> > > BlockIndexRBTree::checkBackups1(BlockIndexRBTree* temp) {
	...
	for (int i = 0; i < temp->BlockMessage->blocks_size(); i++) {
		if (temp->BlockMessage->blocks(i).locs_size() < 3) {
			...
			pair<string, pair<uint64_t, int> > b(temp->name, a);
			q.push(b);
		}
	}
				...
				for (int i = 0; i < temp->BlockMessage->blocks_size(); i++) {
					if (temp->BlockMessage->blocks(i).block().blockid() == blockid) {
						if (backupsDatanodeid.first != "") {
							DatanodeInfo* D = temp->BlockMessage->blocks(i)->add_locs();
							DatanodeID* Did;
							Did = new DatanodeID;

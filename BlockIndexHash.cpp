...
queue<pair<string, pair<uint64_t, int> > > BlockIndexHash::checkBackups() {
	...
	for (int i = 0; i < this->BlockMessage->blocks_size(); i++) {
		if (this->BlockMessage->blocks(i).locs_size() < 3) {
			...
			pair<string, pair<uint64_t, int> > b(this->name, a);
			q.push(b);
		}
	}
		...
		for (int i = 0; i < this->BlockMessage->blocks_size(); i++) {
			if (this->BlockMessage->blocks(i).block().blockid() == blockid) {
				if (backupsDatanodeid.first != "") {
					DatanodeInfo* D = this->BlockMessage->blocks(i)->add_locs();
					DatanodeID* Did;
					Did = new DatanodeID;
